...
class CHIP8():
    def __init__(self, ROM):
        ...
        self.stack = [0] * 16   # CHIP-8 has 16-level stack
        self.stack_pointer = -1  # Means “no active calls yet”
        self.delay_timer = 0
        self.sound_timer = 0
        ...
        # check if it needs to draw to screen
        self.Draw = False
        ...
        #Load ROM automatically to memmory
        if not self.load_ROM(ROM):
            raise Exception("Failed to load ROM.")
        # Guess what, it loads fontset to memory
        self.load_fonts()
    ...
    def load_fonts(self):
        ...
        print("[*] Fonts loaded at 0x{0:03X}..0x{1:03X}".format(self.FONT_START, self.FONT_START + len(self.fontset) - 1))
    
    
    def update_timers(self):
        now = time.time()
        if now - self.last_timer_time >= 1/60:
    ...
    def DRW(self, Vx, Vy, n, memory, I, display):
        VF = 0  # Collision flag
        for row in range(n):
            if I + row >= len(memory):
                break
